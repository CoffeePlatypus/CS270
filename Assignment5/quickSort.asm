...
PART:	addi $sp, $sp, -16	# make room for 4 on stack
	sw $s0, 0($sp)		# store $s0 
	sw $s1, 4($sp)		# store $s1 
	sw $s2, 8($sp)		# store $s2 
	...
	lw $s3, 0($t0)		# pivot = arr[(left + right) / 2]
	
WHILE:	slt $t0, $s1, $s0	# i<=j	$s1<=$s2 !j<i
	bne $t0, $zero, RET	# if $t0 != 0 jump to else
ILOOP:	sll $t0, $s0, 2		# i*4
	add $t0, $t0, $a0	# addr arr[i]
	lw $t0, 0($t0)		# arr[i]
	slt $t0, $t0, $s3	# arr[i] < pivot 
...
JLOOP:	sll $t0, $s1, 2		# j*4
	...
	lw $t0, 0($t0)		# arr[j]
	slt $t0, $s3, $t0	# arr[j] > pivot = pivot < arr[j]
	beq $t0, $zero, IF	# if $t0 == 0 jump to else
	addi $s1, $s1, -1	# j--
	j JLOOP
	
IF:	slt $t0, $s1, $s0	# i<=j	$s1<=$s2 !j<i
